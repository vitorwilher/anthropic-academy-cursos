...
- O **Boletim Focus** de duas formas: resumo **via API** (você hospeda) e
  **via Routine** (Anthropic hospeda)
:::
::: {.column width="42%"}
...
## Primeira chamada: `resumir_focus.py` {.smaller}
...
::: {.callout-note}
Script completo e rodável (com a variante de *tool use*):
`cursos/claude-platform-101/exemplos/resumir_focus.py`
:::
...
## Tool use — garantindo o número certo {.smaller}

No `resumir_focus.py`, em vez de o modelo "lembrar" a mediana, ele **chama uma ferramenta**
que lê o número exato dos dados:
...
## Dois caminhos para o mesmo resumo {.smaller}
...
➡️ A **Routine é o caminho de _managed agent_** — é exatamente aqui que ela entra.

## Posso usar os dois? {.smaller}
...
::: {.callout-tip}
O pulo do gato: do "rodo na minha máquina" (a chamada de API que **você** dispara) para
"**roda na nuvem, no horário, sem mim**" — o managed agent.
:::
...
- O resumo, **dois caminhos**: **API** (você roda `resumir_focus.py`) ou
  **Routine** (managed agent, na nuvem)
